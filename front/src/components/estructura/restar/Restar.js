import { useContext, useState } from 'react'
import { NombreContext } from '../../../context/NombreContext'
import { Button } from 'react-bootstrap'
import { FaRegEye } from "react-icons/fa"
import { FaTrashRestore } from "react-icons/fa";
import axios from 'axios'

export const Restar = () => {

  const { datosUsuario, apellido } = useContext(NombreContext)
  const [nombres, setNombres] = useState(datosUsuario.nombres)
  const [activarEliminadosConfi, setActivarEliminadosConfi] = useState(false)
  const [activarNombresConfi, setActivarNombresConfi] = useState(false)
  const [eliminados, setEliminados] = useState([])
  const [completados, setCompletados] = useState(0)
  const [nombre, setNombre] = useState(nombres[Math.floor(Math.random() * nombres.length)])

  const cambiarNombre = () => { setNombre(nombres[Math.floor(Math.random() * nombres.length)]) }
  const activarEliminados = () => { setActivarEliminadosConfi(!activarEliminadosConfi) }
  const activarNombres = () => { setActivarNombresConfi(!activarNombresConfi) }
  const verNombre = (nombre) => { setNombre(nombre) }
  const sacarNombre = () => {
    if (eliminados.length !== datosUsuario.sacar) {
      eliminados.push(nombre)
      nombres.splice(nombres.indexOf(nombre), 1)
      setEliminados(eliminados)
      setNombres(nombres)
      setCompletados(completados + 1)
      cambiarNombre()
    }
  }
  const sacarEliminadosLista = (index) => {
    nombres.push(eliminados[index])
    eliminados.splice(index, 1)
    setEliminados(eliminados)
    setNombres(nombres)
    setCompletados(completados - 1)
  }
  const sacarNombreLista = (index) => {
    eliminados.push(nombres[index])
    nombres.splice(index, 1)
    setEliminados(eliminados)
    setNombres(nombres)
    setCompletados(completados - 1)
  }
  const finalizar = async () => {
    try {
      const respuesta = await axios.post('/nameapp/restar', { nombres: nombres })
      if (respuesta.data.success) { window.location.reload() }
    } catch (error) {
      console.error('Error al enviar o recibir datos:', error)
    }
  }

  return (
    <div className="container border border-5 rounded m-2">
      <div className="container text-center">
        <div className='d-flex justify-content-between'>
          <div className='btn' onClick={activarEliminados} >
            <p>Eliminados: {eliminados.length}/{datosUsuario.sacar}</p>
          </div>
          <div className='btn' onClick={activarNombres}>
            <p>Nombres: {nombres.length}</p>
          </div>
        </div>
        <h1>{nombre} {apellido}</h1>
        <div className='d-flex justify-content-center'>
          {(completados === datosUsuario.sacar) ? (
            <Button className='m-2' onClick={finalizar}>Finalizar</Button>
          ) : (
            <>
              <Button className='m-2 btn-dark' onClick={sacarNombre}>Sacar</Button>
              <Button className='m-2 btn-dark' onClick={cambiarNombre}>Cambiar</Button>
            </>
          )}
        </div>
        <div className='d-flex justify-content-between'>
          {activarEliminadosConfi && (
            <div className="m-3 text-center">
              <h5>Nombres a sacar</h5>
              {eliminados.length > 0 ? (
                <table className="table table-striped">
                  <thead className='table-primary'>
                    <tr>
                      <th scope="col">#</th>
                      <th scope="col">Nombre</th>
                      <th scope="col">Sacar</th>
                    </tr>
                  </thead>
                  <tbody>
                    {eliminados.map((a, index) => (
                      <tr key={index}>
                        <th scope="row">{++index}</th>
                        <td>{a}</td>
                        <td>
                          <Button className='btn btn-danger' onClick={() => sacarEliminadosLista(--index)}>Sacar</Button>
                        </td>
                      </tr>
                    ))}
                  </tbody>
                </table>
              ) : (
                <p>No hay nombres sacados</p>
              )}
            </div>
          )
          }

          {activarNombresConfi && (
            <div className="m-3 text-center">
              <h5>Todos los nombres</h5>
              {nombres.length > 0 ? (
                <table className="table table-striped">
                  <thead className='table-primary'>
                    <tr>
                      <th scope="col">#</th>
                      <th scope="col">Nombre</th>
                      <th scope="col">Ver</th>
                      <th scope="col">Eliminar</th>
                    </tr>
                  </thead>
                  <tbody>
                    {nombres.map((nombre, index) => (
                      <tr key={index}>
                        <th scope="row">{++index}</th>
                        <td >{nombre}</td>
                        <td>
                          <button className='btn btn-dark' onClick={() => verNombre(nombre)}>
                            <FaRegEye className='m-2' color='rgb(0, 255, 255)' size='1.2rem' />
                          </button>
                        </td>
                        <td>
                          <button className='btn btn-dark' onClick={() => sacarNombreLista(--index)}>
                            <FaTrashRestore className='m-2' color='rgb(255, 0, 255)' size='1.2rem' />
                          </button>
                        </td>
                      </tr>
                    ))}
                  </tbody>
                </table>
              ) : (
                <p>No hay nombres sacados</p>
              )}
            </div>
          )
          }
        </div>
      </div>
    </div>
  )
}
